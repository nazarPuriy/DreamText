...
def rand_poses(size, device, radius_range=[1, 1.5], theta_range=[0, 120], phi_range=[0, 360], return_dirs=False, angle_overhead=30, angle_front=60, jitter=False, uniform_sphere_rate=0.5, img=False):
    ...
    return poses, dirs, angles


def circle_poses(device, radius=1.25, theta=45, phi=0, return_dirs=False, angle_overhead=30, angle_front=60):
...
class NeRFDataset:
    ...
    def collate(self, index):
        ...
        if True:
            ...
            self.img = index[0] < 10 # How many times we train with base image.
        B = len(index)  # always 1

        if self.training:
            # random pose on the fly
            poses, dirs, angles = rand_poses(B, self.device, radius_range=self.opt.radius_range, return_dirs=self.opt.dir_text,
                                     angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front,
                                     jitter=self.opt.jitter_pose, uniform_sphere_rate=self.opt.uniform_sphere_rate, img=self.img)
            ...
            # circle pose
            phi = (index[0] / self.size) * 360 + 30
            poses, dirs = circle_poses(self.device, radius=1., theta=65, phi=phi,
                                       return_dirs=self.opt.dir_text, angle_overhead=self.opt.angle_overhead,
                                       angle_front=self.opt.angle_front)

            # fixed focal
            fov = (self.opt.fovy_range[1] + self.opt.fovy_range[0]) /1.25
