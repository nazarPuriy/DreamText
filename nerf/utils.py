...
class Trainer(object):
    def __init__(self, 
        ...
        # self.optimizer = torch.optim.Adam(nerf.parameters(), lr=5e-4)
        # self.scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[2, 4, 8], gamma=0.5)
    ...
    def prepare_text_embeddings(self):
        ...
        if self.opt.text is None and self.opt.load_token != 1:
    ...
    def train_step(self, data):
        ...
        if self.global_step < self.opt.albedo_iters or data['base']:
            shading = 'albedo'
            ambient_ratio = 1.0
        else: 
            rand = random.random()
        ...
        angles = [data['angle_overhead'], data['angle_front']]
        loss = self.guidance.train_step(text_z, pred_rgb, angles)
